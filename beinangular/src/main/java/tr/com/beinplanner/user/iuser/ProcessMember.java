...
public class ProcessMember  implements ProcessInterface  {
	...
	public List<MenuTbl> getMenuSide(int firmId) {
		List<MenuTbl> menuTbls=menuService.findSideUpperMenuByUserType(UserTypes.USER_TYPE_MEMBER_INT,firmId);
		
		for (MenuTbl menuTbl : menuTbls) {
			List<MenuSubTbl> menuSubTbls=menuService.findSideSubMenuByUserType(UserTypes.USER_TYPE_MEMBER_INT, menuTbl.getMenuId(),firmId);
	...
	public List<MenuTbl> getMenuTop(int firmId) {
		List<MenuTbl> menuTopTbls=menuService.findTopMenuByUserType(UserTypes.USER_TYPE_MEMBER_INT,firmId);
		return menuTopTbls;
	}
	...
	public HmiResultObj canUserDelete(User user) {
		...
		List<PacketSaleFactory> packetSaleFactories= packetSaleService.findUserBoughtPackets(user.getUserId());
